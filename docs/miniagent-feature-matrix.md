...
| Embedding 语义知识检索 | implemented | OpenAI-compatible provider、批量缓存、并发许可交接、向量完整性、余弦与关键词合并排序；显式禁用才仅用FTS | 真实Embedding供应商尚需独立验收 |
...
| Xianyu 闲鱼渠道 | implemented | Core schema 25 渠道会话/设置/投递幂等、按买家持久映射与去重、Web/CLI/Android 管理员 PAT 工作台、普通账号切换、总控与买家会话、自动回复开关与草稿发送、二维码登录、0600 Cookie 持久化、过期停用和飞书告警；Adapter 仅接受回环内部 Bearer 令牌 | 真实账号 E2E 需独立账号、真实 Cookie 或管理员扫码，另行执行咸鱼验收清单；`real-test.mjs` 不包含咸鱼流程。飞书告警需配置知识库 App 凭证与 chat ID |
