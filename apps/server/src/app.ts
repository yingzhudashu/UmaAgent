...
export async function createServer(
  ...
  app.addHook("onRequest", async (request, reply) => {
    ...
    if (
      ...
      request.url === "/api/v2/auth/login" ||
      request.url === "/api/v2/events"
    )
      return;
  ...
  app.post<{ Body: { token?: string } }>("/api/v2/auth/login", async (request, reply) => {
  ...
  app.get("/api/v2/sessions", async () => runtime.listSessions());
  app.post("/api/v2/sessions", async (request) => {
  ...
  app.patch<{ Params: { id: string } }>("/api/v2/sessions/:id", async (request) => {
  ...
  app.delete<{ Params: { id: string } }>("/api/v2/sessions/:id", async (request, reply) => {
    runtime.deleteSession(request.params.id);
    return reply.code(204).send();
  });
  app.post<{ Params: { id: string } }>("/api/v2/sessions/:id/messages", async (request, reply) => {
  ...
  app.post<{ Params: { id: string } }>("/api/v2/sessions/:id/cancel", async (request, reply) => {
  ...
  app.get("/api/v2/models", async () => runtime.listModels());
  app.get("/api/v2/skills", async () => runtime.listSkills());
  app.post("/api/v2/skills/refresh", async () => runtime.refreshSkills());
  app.get("/api/v2/mcp", async () => runtime.mcp.status());
  app.get("/api/v2/knowledge", async () => runtime.knowledge.list());
  app.post<{ Body: { name?: string; path?: string } }>("/api/v2/knowledge", async (request) => {
  ...
  app.get("/api/v2/tasks", async () => runtime.listTasks());
  app.post<{ Body: { prompt?: string; parentSessionId?: string } }>("/api/v2/tasks", async (request) => {
  ...
  app.get<{ Params: { id: string } }>("/api/v2/tasks/:id", async (request) =>
    runtime.getTask(request.params.id),
  );
  app.get<{ Params: { id: string } }>("/api/v2/tasks/:id/snapshot", async (request) =>
    runtime.getSnapshot(runtime.getTask(request.params.id).sessionId),
  );
  app.post<{ Params: { id: string } }>("/api/v2/tasks/:id/cancel", async (request) =>
  ...
  app.delete<{ Params: { id: string } }>("/api/v2/memory/:id", async (request, reply) => {
    runtime.deleteMemoryFact(request.params.id);
    return reply.code(204).send();
  });
  app.get<{ Params: { runId: string } }>("/api/v2/audit/runs/:runId", async (request) =>
    runtime.audit(request.params.runId),
  );
  app.post("/api/v2/uploads", async (request) => {
  ...
  app.get("/api/v2/events", { websocket: true }, (socket, request) => {
    const origin = request.headers.origin;
    if (origin && !allowedOrigin(origin, runtime.config.server.webOrigins)) {
  ...
  app.all("/api/v2/*", async (_request, reply) =>
