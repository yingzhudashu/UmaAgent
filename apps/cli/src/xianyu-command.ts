...
export async function runXianyuCommand(
  ...
  if (action === "status") return print(JSON.stringify(await client.xianyuWorkspace(), null, 2));
  if (action === "start") return void (await client.xianyuStart());
  if (action === "stop") return void (await client.xianyuStop());
  if (action === "pause") return void (await client.xianyuPause());
  if (action === "resume") return void (await client.xianyuResume());
  if (action === "history" || action === "item") {
    const id = positionals[1];
    if (!id) throw new Error(`uma xianyu ${action} <id>`);
    const result = action === "history" ? await client.xianyuHistory(id) : await client.xianyuItem(id);
  ...
  if (action === "chat") {
    ...
    return print(JSON.stringify(await client.xianyuChat({ receiverId, itemId }), null, 2));
  }
  ...
  print(JSON.stringify(await client.xianyuPublish(body), null, 2));
}
