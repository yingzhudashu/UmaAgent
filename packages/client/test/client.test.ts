...
describe("UmaClient", () => {
  ...
  it("covers the complete HTTP facade and optional request shapes", async () => {
    ...
    await client.sendMessage("session/id", "请分析", {
      mode: "agent",
    ...
    const messageRequest = fetchMock.mock.calls.find(
      ([url, init]) =>
        String(url).endsWith("/sessions/session%2Fid/messages") &&
        typeof init?.body === "string" &&
        init.body.includes("attachment/image"),
    );
    expect(messageRequest?.[1]?.body).toContain('"attachmentIds":["attachment/image"]');
