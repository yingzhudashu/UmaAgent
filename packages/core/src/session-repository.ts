...
export class SessionRepository {
  ...
  list(userId?: string): Session[] {
    return rows(
      userId
        ? prepareStatement(this.db, "SELECT * FROM sessions WHERE user_id=? ORDER BY updated_at DESC")
        : prepareStatement(this.db, "SELECT * FROM sessions ORDER BY updated_at DESC"),
      ...(userId ? [userId] : []),
    ).map(toSession);
  }
  ...
  get(id: string): Session {
    const result = row(prepareStatement(this.db, "SELECT * FROM sessions WHERE id = ?"), id);
  ...
  delete(id: string): void {
    const result = prepareStatement(this.db, "DELETE FROM sessions WHERE id = ?").run(id);
