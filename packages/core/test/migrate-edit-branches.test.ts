...
it("migrates legacy edit branches into explicit fork metadata", async () => {
  ...
  expect(
    migrated
      .prepare("SELECT source_message_id FROM conversation_branch_forks WHERE branch_id='legacy-edit'")
      .get(),
  ).toEqual({
...
it("uses the replacement's direct parent as the source for legacy edits", async () => {
  ...
  db.db
    ...
    .run("legacy-edit-parent", session.id, "编辑分支 旧版本", null, Date.now(), Date.now());
  ...
  db.db
    .prepare("UPDATE conversation_branches SET head_message_id=? WHERE id=?")
    .run("replacement", "legacy-edit-parent");
  db.close();
  ...
  expect(
    migrated
      .prepare("SELECT source_message_id FROM conversation_branch_forks WHERE branch_id='legacy-edit-parent'")
      .get(),
  ).toEqual({
