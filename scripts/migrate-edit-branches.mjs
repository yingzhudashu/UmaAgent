...
if (!process.argv[2] || !existsSync(path)) throw new Error("Usage: node scripts/migrate-edit-branches.mjs <state.db>");
const directory = dirname(path);
const release = await lock(directory, { realpath: false, lockfilePath: resolve(directory, "uma.lock"), stale: 10_000, update: 2_000 });
let db;
try {
  ...
  try {
    if (copy.prepare("PRAGMA integrity_check").get().integrity_check !== "ok" || copy.prepare("PRAGMA foreign_key_check").all().length)
  ...
  try {
    ...
    const branches = db.prepare(`SELECT b.id,b.session_id,b.head_message_id,b.created_at
      FROM conversation_branches b LEFT JOIN conversation_branch_forks f ON f.branch_id=b.id
      WHERE b.name <> '主分支' AND b.head_message_id IS NOT NULL AND f.branch_id IS NULL`).all();
    ...
    db.exec("COMMIT");
    console.log(JSON.stringify({ migrated: true, schema: 26, branches: branches.length, backup: backupPath }));
